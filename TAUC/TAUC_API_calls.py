...
def main():
    ...
    sn = input ('Insert Serial Number["2237193A00760"]:')
    rawMAC = input ('Insert Mac Address["5091E3E64470"]:')
    ...
    operation = input('Choose Your Operation[Add, Remove, search]:')
    ...
    if operation.lower() == 'add':
        ...
        request_url = f"/v1/openapi/device-information/device-id?mac={mac}&sn={sn}"
        payload = ""
        method = "GET"
        getdeviceId = push_api(method, conn, request_url, payload, token)
        #print(response)
        deviceID = getdeviceId['result']['deviceId']
        print(deviceID)
        request_url = f"/v1/openapi/device-information/device-info/{deviceID}"
        payload = ""
        method = "GET"
        response = push_api(method, conn, request_url, payload, token)
        print(make_pretty_json(response))
